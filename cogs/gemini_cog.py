                ...
                for i in range(5): # Limit to 5 tool calls to prevent infinite loops
                    ...
                    if tool_name == "google_search_impl":
                        try:
                            ...
                            prompt_parts.append({"text": json.dumps({"tool_response": {"name": "google_search_impl", "response": search_results}})})

                        except GoogleSearchError as se:
                            print(f"Google Search Error: {se}")
                            # Inform the model the tool call failed
                            prompt_parts.append({"text": json.dumps({"tool_response": {"name": "google_search_impl", "response": {"error": str(se)}}})})
                        except Exception as e:
                            print(f"An unexpected error occurred during tool call: {e}")
                            # Pass a generic error back to the model
                            prompt_parts.append({"text": json.dumps({"tool_response": {"name": "google_search_impl", "response": {"error": f"Tool execution failed: {e}"}}})})
                    else:
                        print(f"Warning: Model called unknown tool '{tool_name}'")
                        # Inform the model the tool is not available
                        prompt_parts.append({"text": json.dumps({"tool_response": {"name": tool_name, "response": {"error": "Tool not found."}}})})
